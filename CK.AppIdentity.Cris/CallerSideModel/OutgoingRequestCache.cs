...
namespace CK.AppIdentity.Cris
{
    ...
    /// </para>
    /// </summary>
    public class OutgoingRequestCache
    {
        // Holds the created and not yet completed requests.
        readonly ConcurrentDictionary<ActivityMonitor.LogKey, OutgoingRequest> _cache;
        readonly IPocoFactory<ICrisResultError> _errorFactory;
        readonly PerfectEventSender<IOutgoingRequest, IEvent>? _onEventRelay;

        /// <summary>
        /// Initializes a new <see cref="OutgoingRequestCache"/>. This must be bound and used internally
        /// by an endpoint.
        /// </summary>
        /// <param name="errorFactory">Required error factory (to create an error for a failed <see cref="CrisValidationResult"/>).</param>
        /// <param name="onEventRelay">Optional event sender to which events emitted by commands will be relayed.</param>
        public OutgoingRequestCache( IPocoFactory<ICrisResultError> errorFactory,
                                     PerfectEventSender<IOutgoingRequest,IEvent>? onEventRelay = null )
        {
            _cache = new ConcurrentDictionary<ActivityMonitor.LogKey, OutgoingRequest>();
        ...
        public IEventRequest<T> CreateEvent<T>( IActivityDependentTokenFactory monitor, T e, object? extraData ) where T : class, IEvent
        {
            var issuerToken = monitor.CreateToken( null, dependentTopic: $"Handling '{e.CrisPocoModel.PocoName}' event." );
            return CreateEvent( e, issuerToken, extraData );
        }
        ...
        /// <typeparam name="T">The type of the event.</typeparam>
        /// <param name="e">The event payload.</param>
        /// <param name="issuerToken">The <see cref="IOutgoingRequest.IssuerToken"/>.</param>
        /// <param name="extraData">Request data specific to the endpoint (Authentication token for instance).</param>
        /// <returns>A new event request.</returns>
        public IEventRequest<T> CreateEvent<T>( T e, ActivityMonitor.Token issuerToken, object? extraData ) where T : class, IEvent
        {
            var request = new EventRequest<T>( this, e, issuerToken, extraData );
        ...
        public ICommandRequest<T> CreateCommand<T>( IActivityDependentTokenFactory monitor, T c, object? extraData ) where T : class, IAbstractCommand
        {
        ...
        /// <param name="extraData">Request data specific to the endpoint (Authentication token for instance).</param>
        /// <returns>A new command request.</returns>
        public ICommandRequest<T> CreateCommand<T>( T c, ActivityMonitor.Token issuerToken, object? extraData ) where T : class, IAbstractCommand
        {
            var request = new CommandRequest<T>( this, c, issuerToken, extraData, _onEventRelay );
            Throw.CheckState( _cache.TryAdd( issuerToken.Key, request ) );
            return request;
        }
        ...
        /// <param name="id">The <see cref="IOutgoingRequest.IssuerToken"/> key.</param>
        ...
        internal protected virtual void OnSetSentDate( IParallelLogger logger, OutgoingRequest outgoingRequest )
        {
        }

        internal protected virtual void OnValidationReceived( IParallelLogger logger, OutgoingRequest r )
        {
        }

        internal protected virtual void OnRequestCompleted( IParallelLogger logger, OutgoingRequest outgoingRequest )
        {
        }
