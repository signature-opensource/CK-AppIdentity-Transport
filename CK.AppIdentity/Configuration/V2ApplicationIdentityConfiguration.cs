...
namespace CK.AppIdentity
{
    ...
    /// </summary>
    public sealed class V2ApplicationIdentityConfiguration : V2DomainConfiguration
    {
        readonly string _partyName;

        V2ApplicationIdentityConfiguration( ImmutableConfigurationSection configuration,
                                          string domainName,
        ...
        public static V2ApplicationIdentityConfiguration? Create( IActivityMonitor monitor,
                                                                ...
                                                                string defaultEnvironmentName = CoreApplicationIdentity.DefaultEnvironmentName )
        {
            ...
            var local = V2LocalPartyConfiguration.Create( monitor, root.GetSection( "Local" ), domainName, environmentName, true, ref inheritedProps );
            success = local != null;
            ...
            return new V2ApplicationIdentityConfiguration( root, domainName, fullName, local!, remotes!, ref inheritedProps );
        }

        static V2AppIdentityObjectConfiguration[]? CreateRemotes( IActivityMonitor monitor,
                                                        ...
                                                        ref InheritedConfigurationProps domainProps )
        {
            ...
            var remotes = new List<V2AppIdentityObjectConfiguration>();
            foreach( var c in configuration.GetChildren() )
            {
                var r = Create( monitor, c, domainName, environmentName, allowDomains, ref domainProps, local, remotes );
                if( r == null ) success = false;
                else if( success ) remotes.Add( r );
            }
            return success ? remotes.ToArray() : null;

            static V2AppIdentityObjectConfiguration? Create( IActivityMonitor monitor,
                                    ImmutableConfigurationSection configuration,
                                    string? appDomainName,
                                    string? appEnvironmentName,
                                    bool allowDomain,
                                    ref InheritedConfigurationProps domainProps,
                                    V2LocalPartyConfiguration? localToCheckName,
                                    IEnumerable<V2AppIdentityObjectConfiguration> remotesToCheckHomonyms )
            {
                ...
                if( remotesSection.Exists() )
                {
                    ...
                    var local = V2LocalPartyConfiguration.Create( monitor,
                    ...
                    return success
                            ? new V2DomainConfiguration( configuration, domainName, fullName, local!, remotes!, ref inhProps )
                            : null;
                }
                else
                {
                    ...
                    return success
                            ? new V2RemotePartyConfiguration( configuration, domainName, fullName, configuration["Address"], ref remoteProps )
                            : null;
                }
