...
## Message Prefix: Encoding and Length

All messages exchanged by the Transport layer are prefixed by the message's encoding and length.
- The prefix starts with a first byte: `|L0|L1|R|P0|P1|P2|P3|P4|`.
  ...
  - P0-P4 bits is the Encoding, a number between 0 and 32. This number defines the "type" of the
...
At a higher level [Encodings](MessageEncoding.cs) are identified by a string (its unique name).
How the message is encoded can also define how messages are exchanged: an "Encoding" can be considered a "Communication Protocol". By merging
these 2 concepts here, we greatly simplify the implementation and the understandability of the Transport layer.

The "0 Protocol" encoding name is reserved: this is the encoding of CK.AppIdentity.TransportLayer itself.
This encoding comes with a protocol that handles special messages used to negotiate, accept, reject incoming parties
and outgoing connections.
This lets 31 possible encodings. This may seem a limitation however this limit applies to a Remote party pair: there
can be any number of possible encodings, among them 2 parties that start to interact initially negotiate the ones they
can and want to use up to 31 different encodings (or said differently, any Transport between 2 parties can support up
to 31 different protocols).

The final length of a message on the wire is between 2 bytes (the special Empty message, see below) and 2 Gib.
A minimal 1 byte message requires 3 bytes (in any of the 31 available encodings). 
...
- The `TransportMessage.Canceled` is a second message invalid singleton, that is used to signal a canceled read operation.
- The `TransportMessage.Empty` is a valid singleton that can be exchanged: it is used
for KeepAlive messages. Its length on the wire is 2 bytes and is the shortest message that exists. Regular encodings
